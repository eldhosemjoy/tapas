...
def create_int_feature(values):
  ...
  return (np.asarray(values,dtype=np.int32)).reshape(1,len(values))
...
def create_string_feature_question(values):
  return (np.asarray(values)).reshape(1,len(values))
...
class ToClassifierTensorflowExample(ToTrimmedTensorflowExample):
  ...
  def convert(self, interaction,
    ...
    features['question_id'] = create_string_feature_question(
    ...
    # print("Returning TF Example Features",features,len(features))
    # for item in features:
    #   print("Item",type(features[item]),features[item],features[item].shape)
    #   exit(0)
