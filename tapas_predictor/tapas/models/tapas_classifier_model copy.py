...
def compute_column_logits(output_layer,
  ...
  """
  hidden_size = output_layer.shape.as_list()[-1]
  with tf.variable_scope("column_output_weights", reuse=tf.AUTO_REUSE):
    column_output_weights = tf.get_variable(
        "column_output_weights", [hidden_size],
        initializer=tf.zeros_initializer()
        if init_cell_selection_weights_to_zero else _classification_initializer())
  with tf.variable_scope("column_output_bias", reuse=tf.AUTO_REUSE):
...
def _get_classification_outputs(config,
  ...
  print("row_index : ",row_index, type(row_index))
  print("col_index : ",col_index, type(col_index))
...
def model_fn_builder(config):
  ...
  def model_fn(features, labels, mode, params):
    ...
    del labels  # Unused.
    print("Inside Model Function", mode)
    ...
    print("label_ids : ",type(label_ids))
    ...
    if config.use_answer_as_supervision:
      ...
      print("All in an else condition")
      answer = None
      numeric_values = None
    ...
    if init_checkpoint:
      ...
      if config.use_tpu:
        ...
        print("else loading checkpoint", init_checkpoint, assignment_map)
        tf.train.init_from_checkpoint(init_checkpoint, assignment_map)
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      print("Predictions Before Passing : ",predictions)
      output_spec = tf.estimator.tpu.TPUEstimatorSpec(
